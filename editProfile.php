                                ...
                                <div class="padding-border solid-border">
                                    ...
                                    <form action="<?php echo SERVER; ?>/controller/editInfoSuccess.php" method="post">
                                        <div class="form-group">
                                            <label>Name</label>
                                            <input type="text" class="onlyChars form-control" name="userName" value="<?php echo htmlentities(stripslashes($proInfo['u_fname'])), " ", htmlentities(stripslashes($proInfo['u_lname'])); ?>" pattern="(?=.*[a-zA-Z])+([.,\s])*.{2,}" title="Only letters, space, dot and comma allowed"/>
                                        </div>
                                        <div class="form-group">
                                            <label>Email</label>
                                            <input type="email" class="form-control" name="email" value="<?php echo htmlentities(stripslashes($proInfo['u_email'])); ?>" pattern="[([\w-]+(?:\.[\w-]+)*)@((?:[\w-]+\.)*\w[\w-]{0,66})\.([a-z]{2,6}(?:\.[a-z]{2})?)]i" title="Please insert a valid email" />
                                        </div>
                                        <div class="form-group">
                                            <label>Contact</label>
                                            <input type="tel" class="phnNum form-control" name="contact" id="phone" value="<?php echo htmlentities(stripslashes($proInfo['u_contact'])); ?>" />
                                        </div>
                                        <br/>
                                        <div class="text-center">
                                            <button type="submit" class="btn btn-success" name="editInfo" pattern="(?=.*[0-9])+.{5,}" title="please insert a valid Phone Number">Update</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
